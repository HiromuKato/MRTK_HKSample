...
namespace MRTK_HKSample
{
    ...
    /// ハンドフォトキャプチャ
    /// </summary>
    public class HandPhotoCapture : MonoBehaviour
    {
        [SerializeField] 
        private PhotoCaptureExample photoCapture = default;
        ...
        void Update()
        {
            ...
            if(CheckCondition(angleLeft, angleRight))
            {
                TakePhoto();
            }
        ...
        /// <summary>
        /// 写真撮影を開始するための条件となるアルゴリズム
        /// とりあえず、左右の人差し指・親指の角度が共に60-90度の範囲になったらtrueを返す簡単な処理
        /// 厳密に行うにはここの処理を工夫する
        /// </summary>
        /// <param name="angleLeft"></param>
        /// <param name="angleRight"></param>
        /// <returns></returns>
        private bool CheckCondition(float angleLeft, float angleRight)
        {
            bool result = false;

            if (60 < angleLeft && angleLeft < 90 &&
                60 < angleRight && angleRight < 90)
            {
                result = true;
            }
